...
//   const totalScore = apiResponse.items.reduce((acc, item) => acc + item.score, 0);
...
//                 <Text style={[styles.score, { color: props.color }]}>{item.score.toFixed(2)}</Text>
//               </View>
//             )}
...
//           <Text style={styles.totalScore}>{totalScore.toFixed(2)} CO2 kg/product kg</Text>
//         </View>
//       </List.Section>
//     </View>
//   );
...
const History = () => {
  // Calculate the total carbon score
  const totalScore = apiResponse.items.reduce((acc, item) => acc + item.score * item.kg, 0);

  // Calculate stars based on total carbon score (lower score = more stars)
  const getStars = (score) => {
    const maxScore = 4; // Define maximum score for scaling purposes
